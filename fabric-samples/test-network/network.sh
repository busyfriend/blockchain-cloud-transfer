...
function printHelp() {
  ...
  echo "      - 'up createChannel' - bring up fabric network with two channel"
  echo "      - 'createChannel' - create and join a channel after the network is created"
  echo "      - 'deployCloudChaincode' - deploy the cloud chaincode on the channel"
  echo "      - 'down' - clear the network with docker-compose down"
  echo "      - 'restart' - restart the network"
  echo
  echo "    Flags:"
  echo "    -ca <use CAs> -  create Certificate Authorities to generate the crypto material"
  echo "    -c1 <channel name> - channel name to use (defaults to \"mychannel12\")"
  echo "    -c2 <channel name> - channel name to use (defaults to \"mychannel34\")"
...
function createConsortium() {
  ...
  #configtxgen -profile Genesis24 -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
## Bring up the network if it is not arleady up.
 ...
 scripts/createChannel.sh $CHANNEL_NAME_1 $CHANNEL_NAME_2 $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
...
function deployCloudChaincode() {

  #scripts/deployCloudChaincode.sh $CHANNEL_NAME_1 $CHANNEL_NAME_2 $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
  scripts/deployCloudChaincode.sh -c1 $CHANNEL_NAME_1 -c2 $CHANNEL_NAME_2 -l $CC_SRC_LANGUAGE -v $VERSION -d $CLI_DELAY -r $MAX_RETRY
  #scripts/packageInstall.sh -c1 $CHANNEL_NAME_1 -c2 $CHANNEL_NAME_2 -l $CC_SRC_LANGUAGE -v $VERSION
  #scripts/approveCommitInit.sh -c1 $CHANNEL_NAME_1 -c2 $CHANNEL_NAME_2 -v $VERSION -r $MAX_RETRY -d $CLI_DELAY
  if [ $? -ne 0 ]; then
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt cloud.tar.gz cloud
  fi
}
...
CLI_DELAY=3
# channel name defaults to "mychannel12" and "mychannel34"
...
if [ "$MODE" == "up" ]; then
  ...
  echo
elif [ "$MODE" == "deployCloudChaincode" ]; then
  echo "deploying chaincode on channel '${CHANNEL_NAME_1}' and '${CHANNEL_NAME_2}'"
  echo
else
  printHelp
  exit 1
